...
static void message_handler(rq_message_t *msg, void *arg)
{
	...
	printf("http config: received message\n");
...
static void cmdHost(control_t *ptr, const risp_length_t length, const risp_data_t *data)
{
	assert(ptr);
	assert(length > 0);
	assert(data != NULL);
...
static void cmdPath(control_t *ptr, const risp_length_t length, const risp_data_t *data)
{
	assert(ptr);
	assert(length > 0);
	assert(data != NULL);
